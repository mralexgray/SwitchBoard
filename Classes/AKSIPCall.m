...
- (void) record
{
	self.mediaFlow = [AKSIPCallMediaFlow.alloc initWithCall:self
													playing:nil
												recordingTo:@"/Users/localadmin/Desktop/record.wav"];
	_mediaFlow.whatToPlay = nil;
}
- (void) play
{
	NSString *tiny = [[NSBundle.mainBundle resourcePath] stringByAppendingPathComponent:@"tiny_wings.wav"];
	self.mediaFlow = [AKSIPCallMediaFlow.alloc initWithCall:self
													playing:tiny
												recordingTo:@"/Users/localadmin/Desktop/record.wav"];
}
...
- (void)answer {
    ...
    if (status != PJ_SUCCESS) NSLog(@"Error answering call %@", self);
}
- (void)hangUp {
